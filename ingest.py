...
def create_index(client, index_name):
    ...
    print(
        f"Index '{index_name}' berhasil dibuat dengan engine Lucene dan Indonesian Analyzer!"
    )


def run_etl(dataset_path="dataset/Sunnah_v2.csv"):
    ...
    if has_grade:
        print("  Kolom 'Grade' terdeteksi, akan disimpan ke index.")
        ...
        for j, (_, row) in enumerate(batch_df.iterrows()):
            action = {
                ...
                "_source": {
                    ...
                    "terjemahan": row[
                        "Terjemahan"
                    ],  # Teks utuh disimpan untuk Highlighting & BM25
                    "embedding": embeddings[j].tolist(),
                },
            }
            if has_grade:
                action["_source"]["grade"] = row["Grade"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Ingest hadith dataset into OpenSearch"
    )
    parser.add_argument(
        "--dataset",
        default="dataset/Sunnah_v2.csv",
        help="Path to dataset CSV (default: dataset/Sunnah_v2.csv)",
    )
    args = parser.parse_args()
    run_etl(args.dataset)
